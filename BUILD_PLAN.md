...
|---|---|---|
| 2.1 | Welcome / Product Philosophy | **Written, not yet CI-confirmed (this pass).** States plainly that Missions restrict phone function and that higher tiers include confrontational language by choice, checked against `FallbackVoiceBank`'s actual tier scoping (Operator/Warden/Iron, never Recruit) so that claim describes real app behavior rather than aspirational copy. No urgency/dark-pattern language — `welcome_fit_body` explicitly reassures rather than pressures, reviewed against Architecture doc §4.1's "deceptive/manipulative design patterns" app-review risk and the §4 cross-cutting behavior-vs-identity checklist. No persistence — pure disclosure screen, no `User` field or DAO call involved, so no DAO round-trip test file exists for it (see `WelcomeFragment`'s kdoc for why that's a deliberate omission, not a gap). |
